   ...
   // private static final UUID Power_Service = UUID.fromString("19B10010-E8F2-537E-4F6C-D104768A1214");
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()){
            ...
            default:
                ...
                mConnectedGatt = device.connectGatt(this,false,mGattCallback); // or true ?
                //Display progress UI
    ...
    private BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
        ...
        public void onServicesDiscovered(BluetoothGatt gatt, int status) {
            ...
            mHandler.sendMessage(Message.obtain(null, MSG_PROGRESS, "Services discovered"));
            for (BluetoothGattService service : services) {
                List<BluetoothGattCharacteristic> characteristics = service.getCharacteristics();
                Log.i(TAG, "Service: " + service.getUuid());
                for(BluetoothGattCharacteristic characteristic: service.getCharacteristics()){
                    Log.i(TAG,"UUID of the Characteristic of Service : " + characteristic.getUuid());
                }
            }
        ...
        public void onCharacteristicChanged(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic) {
            ...
            Log.i(TAG,"onCharacteristicChanged: " + characteristic.getUuid() + characteristic.getProperties());
    ...
    private void updatePowerValue(BluetoothGattCharacteristic characteristic){
        // see how can it be handled later (for now just check if it is reading anything from ardunio)
        long value = characteristic.getIntValue(BluetoothGattCharacteristic.FORMAT_UINT32, 0) & 0x00000000ffffffffL;
        //Log.i(TAG,"updatePowerValue second trial" + (java.nio.ByteBuffer.wrap(characteristic.getValue()).getInt()));
       // Log.i(TAG,"updatePowerValue second trial" + String.format("%.0f",java.nio.ByteBuffer.wrap(characteristic.getValue()).getInt()));

        //Big Indian
        //mPower.setText(String.format("%.0f",java.nio.ByteBuffer.wrap(characteristic.getValue()).getInt()));
        //Little Indian
        //java.nio.ByteBuffer.wrap(characteristic.getValue()).order(java.nio.ByteOrder.LITTLE_ENDIAN).getInt();
    }
